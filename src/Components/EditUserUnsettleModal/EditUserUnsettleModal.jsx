...
import './EditUserUnsettleModal.css';

const EditUserUnsettleModal = ({ show, handleClose, firstName, lastName, unsettle, userId }) => {
  ...
  const generateLink = async () => {
    ...
    try {
      ...
      const currentBalance = parseFloat(userData.unsettle);
      ...
      if (response.ok) {
        ...
        setFlashMessage(`Unsettle ${isAdding ? 'added to' : 'subtracted from'} user successfully!`);
  ...
  return (
    <Modal show={show} onHide={handleClose}>
      <Modal.Header closeButton>
        <Modal.Title className="text-center">Update {`${lastName || "Users's"}`} Unsettle</Modal.Title>
      </Modal.Header>
      <Modal.Body>
        <div className="container-fluid">
          ...
          <div className="row">
            ...
            <div className="col-md-6 mb-4">
              <label>{firstName || "User"} Unsettle </label>
              <input
                type="text"
                name="lastName"
                className="form-control rounded-end-5 border border-3"
                placeholder="Unsettle"
                value={unsettle || ""}
                readOnly
              />
            </div>
...
EditUserUnsettleModal.propTypes = {
  show: PropTypes.bool.isRequired,
  handleClose: PropTypes.func.isRequired,
};

export default EditUserUnsettleModal;
